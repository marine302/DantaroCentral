...
class RedisManager:
    ...
    def _deserialize_data(self, data: bytes) -> Any:
        """바이트 데이터를 역직렬화."""
        if self.enabled and isinstance(data, bytes):
    ...
    def cache_recommendations(self, recommendations: List[Dict], cache_type: str = "default") -> bool:
        ...
        try:
            ...
            if self.enabled:
                result = self.redis_client.setex(key, self.config.RECOMMENDATIONS_TTL, data)
                return bool(result)
            else:
    ...
    def cache_support_levels(self, symbol: str, support_data: Dict) -> bool:
        ...
        try:
            ...
            if self.enabled:
                result = self.redis_client.setex(key, self.config.SUPPORT_LEVELS_TTL, data)
                return bool(result)
            else:
                self._memory_cache[key] = {
                    'data': data,
    ...
    def cache_market_status(self, market_data: Dict) -> bool:
        ...
        try:
            ...
            if self.enabled:
                result = self.redis_client.setex(key, self.config.MARKET_STATUS_TTL, data)
                return bool(result)
            else:
                self._memory_cache[key] = {
                    'data': data,
    ...
    def set_worker_status(self, worker_id: str, status_data: Dict) -> bool:
        ...
        try:
            ...
            if self.enabled:
                result = self.redis_client.setex(key, 300, data)  # 5분 TTL
                return bool(result)
            else:
                self._memory_cache[key] = {
                    'data': data,
                ...
                try:
                    ...
                    for key in keys:
                        if isinstance(key, bytes):
                            ...
                            key_str = key
                        worker_id = key_str.replace(self.config.WORKER_STATUS_PREFIX, '')
                        data = self.redis_client.get(key)
                        if data:
                            result[worker_id] = self._deserialize_data(data)
                except Exception as e:
                    print(f"Error retrieving worker keys: {e}")
            else:
                # 메모리 캐시에서 워커 상태 조회
                for key, cache_entry in list(self._memory_cache.items()):
